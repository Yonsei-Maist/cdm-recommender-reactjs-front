        ...
        data.cdmWordsList.length !== 0 && (
          <div className="d-flex flex-column">
            <div className="d-flex flex-row justify-content-between">
              <h3>
                <MDBBadge color="warning">{data.emrWordId}</MDBBadge>
              </h3>
              {data.markedWord && data.markedWord.boolIsChanged && (
                <MDBBtn
                  ...
                  color="primary"
                  onClick={() =>
                    handleOnClickResetButton(data.cdmWordsList, data.markedWord)
                  }
                >
                  Reset
                </MDBBtn>
            ...
            >
              <MDBListGroup>
                {data.cdmWordsList.map(
                  ({ cdmWordId, floatSimilarity, detail }) => {
                    return (
                      <MDBListGroupItem
                        hover
                        style={{
                      ...
                      >
                        {detail.word} - {floatSimilarity * 100}%
                      </MDBListGroupItem>
                    );
                  }
                )}
